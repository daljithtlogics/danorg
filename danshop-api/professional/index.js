...
app.get('/edit/:id', async (req, res) => { 
  ...
  try {
    ...
    const sqlQuery = "SELECT * FROM membership WHERE UPPER(choose_membership)='COMMERCIAL' AND UPPER(membertype)='PROFESSIONAL'"+` AND id=${id}`;	 			  		 		     	    
...
app.get('/comments/:id', async (req, res) => { 
  ...
  try {
    const stream = await createSshTunnel(sshTunnelConfig, dbServer);	  	
    const sqlQuery = `SELECT c.description,c.created_at,r.title,s.title AS reason_title FROM professional_comments as c left join sub_reasons as s on s.id=c.subreason_id left join main_reasons as r on r.id=s.main_id WHERE c.row_id=${id}`;	     	    
    const results = await queryDatabase(sqlQuery,stream);													  							  		
    res.status(200).json(results);   						
  } catch (error) {
    res.status(500).json({ error: 'Error fetching data from database: ' + error.message });		 		 		    						   	
...
app.post('/add', async (req, res) => {			
  ...
  const sqlQuery = "SELECT * FROM membership WHERE UPPER(choose_membership)='COMMERCIAL' AND "+`UPPER(membertype)='${type}' AND member_no='${memberNo}'`;	    
  ...
  else if (type=='PROFESSIONAL' && (!data.prof_type || data.prof_type === "")) {
    return res.status(500).json({ error: 'Professional Type is Missing','error_field':'prof_type' });		
  } 
  ...
  try {
	...
	const professionalType=(typeof data.prof_type=="undefined")?'':data.prof_type;     
	...
	joindate,card_expiry,choose_membership,professional_type) VALUES ('${infoTitle}','${firstName}','${surName}','${infoDob}',
	...
	'${accountType}','${accountNo}','${sign}','${debt}','${paid}','${startDate}','${joinDate}','${expireDate}','COMMERCIAL','${professionalType}')`;  				
	...
	if(insert_id >0)	
	{
		...
		const tblName = (type=='PROFESSIONAL')?'professional_comments':'individual_comments';		
		
		const stream = await createSshTunnel(sshTunnelConfig, dbServer);   
		const sqlQuery = `INSERT INTO ${tblName} (row_id,subreason_id,description) VALUES ('${insert_id}','${subReason}','${comment}')`;               
...
app.post('/update/:id', async (req, res) => { 
  const id = req.params.id; 
  const data = req.body;  
  const chktype=(typeof data.member_type=="undefined")?'PROFESSIONAL':data.member_type; 
  const text = `'${chktype}'`;  
  const stream = await createSshTunnel(sshTunnelConfig, dbServer);	     				
  ...
  else if (chktype=='PROFESSIONAL' && (!data.prof_type || data.prof_type === "")) {
    return res.status(500).json({ error: 'Professional Type is Missing','error_field':'prof_type' });		
  } 
  else if (!data.main_reason || data.main_reason === "") {
    return res.status(500).json({ error: 'Main Reason is Missing','error_field':'main_reason' });	  	  
  ...
  if(result.length==1)  
  {
	  try {  
		...
		startdate='${startDate}',capturedate='${captreDate}',joindate='${joinDate}',updatedate='${upDae}',
		card_expiry='${expireDate}',professional_type='${professionalType}' WHERE id=${id}`;  
		...
		if(id >0)	 
		{
			...
			const stream = await createSshTunnel(sshTunnelConfig, dbServer);   
			const sqlQuery = `INSERT INTO ${tblName} (row_id,subreason_id,description) VALUES ('${id}','${subReason}','${comment}')`;               
