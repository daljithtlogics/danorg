...
app.post('/add', async (req, res) => {			
  ...
  if (!data.row_id || data.row_id === "") {   
    ...
    return res.status(500).json({ error: 'Passport Number is Missing','error_field':'passport_no' });  			
  } 
   ...
   else if (!data.provider_country || data.provider_country === "") {
    return res.status(500).json({ error: 'Country is Missing','error_field':'provider_country' });    	  		
  } 
