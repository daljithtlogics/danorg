...
app.get('/get', async (req, res) => {
  try {
    ...
    const sqlQuery = `SELECT * FROM membership WHERE UPPER(membertype)='NON_DIVER'`;	 	   	
...
app.get('/edit/:id', async (req, res) => {   
  ...
  try {
    const stream = await createSshTunnel(sshTunnelConfig, dbServer);		
    const sqlQuery = `SELECT * FROM membership WHERE id=${id} AND UPPER(membertype)='NON_DIVER'`;    	    
    const results = await queryDatabase(sqlQuery, stream);		  		  		
    res.status(200).json(results);   			
  } catch (error) {
    res.status(500).json({ error: 'Error fetching data from database: ' + error.message });		    	
...
app.post('/add', async (req, res) => {			
  ...
  const stream = await createSshTunnel(sshTunnelConfig, dbServer);	    
  const sqlQuery = "SELECT (IFNULL(MAX(id),0)+1) AS max_id FROM membership WHERE UPPER(choose_membership)='ANNUAL' AND UPPER(membertype)='NON_DIVER'";   	
  ...
  try {
    ...
    const sqlQuery = `INSERT INTO membership (parent_id,title1,fname1,sname1,passport1,member_no,relation,telephone1,telephone2,cellphone1,email1,status1,dob1,capturedate,updatedate,membertype,choose_membership) 
	VALUES ('${row_id}','${title}','${fname}','${sname}','${passport}','${memberNO}','${relation}','${phonehome}','${phonework}','${cellphone}','${email}','${status}','${dob}','${capturedate}','${updatedate}','non_diver','annual')`;  
...
app.post('/update/:id', async (req, res) => { 
  ...
  if(result.length==1)  
  {
	  try {  
			...
			status1='${status}',dob1='${dob}',capturedate='${capturedate}',updatedate='${updatedate}' WHERE id=${id}`;   				
