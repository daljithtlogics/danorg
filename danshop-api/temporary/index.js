...
app.get('/get', async (req, res) => {
  try {
    ...
    const sqlQuery = "SELECT * FROM membership WHERE UPPER(choose_membership)='TEMPORARY'";			
...
app.get('/get_members', async (req, res) => {
  try {
	...
	{
		text += `'%${search}%'`;					
		query += "SELECT count(*) AS record_count FROM membership WHERE UPPER(choose_membership)='TEMPORARY'"+` AND (CONCAT(fname1,' ',sname1) LIKE ${text} OR member_no LIKE ${text} OR email1 LIKE ${text} OR status1 LIKE ${text} OR DATE_FORMAT(startdate,"%d/%m/%Y") LIKE ${text})`;       		
		sqlQuery += "SELECT * FROM membership WHERE UPPER(choose_membership)='TEMPORARY'"+` AND (CONCAT(fname1,' ',sname1) LIKE ${text} OR member_no LIKE ${text} OR email1 LIKE ${text} OR status1 LIKE ${text} OR DATE_FORMAT(startdate,"%d/%m/%Y") LIKE ${text}) LIMIT ${start},${end}`;    
	}
	else
	{
		query += `SELECT count(*) AS record_count FROM membership WHERE UPPER(choose_membership)='TEMPORARY'`;	  	 			 
		sqlQuery += `SELECT * FROM membership WHERE UPPER(choose_membership)='TEMPORARY' LIMIT ${start},${end}`;	 			        		           
	} 
...
app.get('/comments/:id', async (req, res) => { 
  ...
  try {
    const stream = await createSshTunnel(sshTunnelConfig, dbServer);	  	
    const sqlQuery = `SELECT c.description,c.created_at,r.title,s.title AS reason_title FROM temporary_comments as c left join sub_reasons as s on s.id=c.subreason_id left join main_reasons as r on r.id=s.main_id WHERE c.row_id=${id}`;	     	    
    const results = await queryDatabase(sqlQuery,stream);											  							  		
    res.status(200).json(results);   						
  } catch (error) {
    res.status(500).json({ error: 'Error fetching data from database: ' + error.message });		 				   	
...
app.post('/add', async (req, res) => {			
  ...
  const stream = await createSshTunnel(sshTunnelConfig, dbServer);	    
  const sqlQuery = "SELECT (IFNULL(MAX(id),0)+1) AS max_id FROM membership WHERE UPPER(choose_membership)='TEMPORARY'";	   	
  const result = await queryDatabase(sqlQuery, stream);	  
  ...
  try {
    ...
    const memberNO = `DANE ${next_id}`; // Generate member_id with a random number   
	...
	const kinDob=(typeof data.kinDob=="undefined" || data.kinDob== null || data.kinDob=='')?'0000:00:00':data.kinDob;  	
	const startDate=(typeof data.startDate=="undefined" || data.startDate== null || data.startDate=='')?'0000:00:00':data.startDate;      
	const endDate=(typeof data.endDate=="undefined" || data.endDate== null || data.endDate=='')?'0000:00:00':data.endDate;  
	
    const sqlQuery = `INSERT INTO membership (title1,fname1,sname1,passport1,member_no,address1,address2,city1,province1,postcode1,country1,telephone1,telephone2,cellphone1,fax,email1,medical_aid,medical_aid_no,passport2,agency,dive_location,spear_diver,creational,qualification,relation,title2,fname2,sname2,address3,address4,city2,province2,postcode2,country2,telephone3,telephone4,cellphone2,email2,status1,dob1,dob2,startdate,lastdate,paid,choose_membership) 
	...
	'${kinCity}','${kinProvince}','${kinPostCode}','${kinCountry}','${kinTelHome}','${kinTelWork}','${kinCellPhone}','${kinEmail}','${status}','${infoDob}','${kinDob}','${startDate}','${endDate}','${paid}','temporary')`;  
	...
	if(insert_id >0)	
	{
		...
		const stream = await createSshTunnel(sshTunnelConfig, dbServer);   
		const sqlQuery = `INSERT INTO temporary_comments (row_id,subreason_id,description) VALUES ('${insert_id}','${subReason}','${comment}')`;               
...
app.post('/update/:id', async (req, res) => { 
  ...
  const result = await queryDatabase(sqlQuery, stream);			
	...
	if (!data.f_name || data.f_name === "") {
	...
	else if (!data.dive_loc || data.dive_loc === "") {
		return res.status(500).json({ error: 'Dive location is Missing','error_field':'dive_loc' });		
	}
	else if (!data.qualification || data.qualification === "") {
		return res.status(500).json({ error: 'Qualification is Missing','error_field':'qualification' });  		
	}   
	...
	else if (!data.kin_name || data.kin_name === "") {  
		return res.status(500).json({ error: 'First name is Missing','error_field':'kin_name' });        
	}   
		...
		return res.status(500).json({ error: 'Relationship is Missing','error_field':'kin_relation' });        
	}   
  ...
  if(result.length==1)  
  {
	  try {  
		...
		const infoPassport=(typeof data.info_pass=="undefined" || data.info_pass== null)?'':data.info_pass;    	
		const infoAddress1=(typeof data.info_addr1=="undefined" || data.info_addr1== null)?'':data.info_addr1;  	
		const infoAddress2=(typeof data.info_addr2=="undefined" || data.info_addr2== null)?'':data.info_addr2;    
		...
		const diveLocation=(typeof data.dive_loc=="undefined")?'':data.dive_loc;    
		const qualification=(typeof data.qualification=="undefined")?'':data.qualification;   	
		const agency=(typeof data.agency=="undefined")?'':data.agency;  
		const freeDiver=(typeof data.spear_dive=="undefined")?'':data.spear_dive;  
		const creationDiver=(typeof data.recr_dive=="undefined")?'':data.recr_dive;  

		const kinTitle=(typeof data.kin_title=="undefined")?'':data.kin_title;    
		const kinFirstName=(typeof data.kin_name=="undefined")?'':data.kin_name;  	
		const kinSurName=(typeof data.kin_sname=="undefined")?'':data.kin_sname;  
		const kinPassport=(typeof data.kin_passport=="undefined")?'':data.kin_passport;   
		const kinRelation=(typeof data.kin_relation=="undefined")?'':data.kin_relation;  	 		
		const kinAddress1=(typeof data.kin_address1=="undefined")?'':data.kin_address1;   	
		const kinAddress2=(typeof data.kin_address2=="undefined")?'':data.kin_address2;  

		const kinCity=(typeof data.kin_city=="undefined")?'':data.kin_city;	 
		const kinProvince=(typeof data.kin_province=="undefined")?'':data.kin_province;  
		const kinPostCode=(typeof data.kin_post=="undefined")?'':data.kin_post;      
		const kinCountry=(typeof data.kin_country=="undefined")?'':data.kin_country;	  
		const kinTelHome=(typeof data.kin_tel=="undefined")?'':data.kin_tel; 
		const kinTelWork=(typeof data.kin_num=="undefined")?'':data.kin_num;  
		const kinCellPhone=(typeof data.kin_phone=="undefined")?'':data.kin_phone;  
		const kinEmail=(typeof data.kin_email=="undefined")?'':data.kin_email;  
		const mainReason=(typeof data.mainReason=="undefined")?'':data.mainReason;  	
		const subReason=(typeof data.subReason=="undefined")?'':data.subReason; 
		const comment=(typeof data.comment=="undefined")?'':data.comment; 	
		...
		const kinDob=(typeof data.kin_dob=="undefined" || data.kin_dob== null || data.kin_dob=='')?'0000:00:00':data.kin_dob;  	
		...
		const sqlQuery = `UPDATE membership SET title1='${infoTitle}',fname1='${firstName}',sname1='${sirName}',passport1='${infoPassport}',address1='${infoAddress1}',address2='${infoAddress2}',city1='${infoCity}',province1='${infoProvince}',postcode1='${postCode}',qualification='${qualification}',creational='${creationDiver}',lastdate='${endDate}',  
		capturedate='${captreDate}',updatedate='${upDae}',paid='${paid}',country1='${infoCountry}',telephone1='${telHome}',telephone2='${telWork}',cellphone1='${cellPhone}',fax='${fax}',status1='${status}',dive_location='${diveLocation}',dob1='${infoDob}',email1='${infoEmail}',medical_aid='${medicalAid}',medical_aid_no='${medicalAidNo}',agency='${agency}',
		spear_diver='${freeDiver}',title2='${kinTitle}',fname2='${kinFirstName}',sname2='${kinSurName}',startdate='${startDate}',dob2='${kinDob}',passport2='${kinPassport}',relation='${kinRelation}',address3='${kinAddress1}',address4='${kinAddress2}',city2='${kinCity}',province2='${kinProvince}',postcode2='${kinPostCode}',country2='${kinCountry}',telephone3='${kinTelHome}',  
		telephone4='${kinTelWork}',cellphone2='${kinCellPhone}',email2='${kinEmail}' WHERE id=${id}`;   				
		...
		if(id >0)	 
		{
			// Send a success response	  		
			const stream = await createSshTunnel(sshTunnelConfig, dbServer);   
			const sqlQuery = `INSERT INTO temporary_comments (row_id,subreason_id,description) VALUES ('${id}','${subReason}','${comment}')`;               
