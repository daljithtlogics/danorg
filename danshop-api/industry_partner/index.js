...
app.get('/get_members/:id', async (req, res) => {    
  ...
  try {
    ...
    const sqlQuery = `SELECT id,fname1,sname1,member_no,email1 from membership WHERE parent_id=${id} AND membertype='industry_partner'`;	  	     	    
...
app.post('/add', async (req, res) => {			
  ...
  try {
	...
	'${country}','${phonehome}','${cellphone}','${email_address}','${new_pwd}','${medical_aid}','${medical_no}','${memberNO}','verified','not_paid','industry_partner','ACTIVE','${start_date}','${start_date}','${parent_id}','annual')`;  
