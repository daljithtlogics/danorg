...
@Component({
  selector: 'commercial',			
  templateUrl: './commercialmemberships.component.html',							
  styleUrls: ['./membershipcommon.component.css'],      
  standalone: true,
  imports: [SharedModule],
})

export class CommercialmembershipsComponent implements OnInit 	
{
  greeting = 'commercial';
  pageSize : number;  
  maxPages: number;		 
  ...
  getData(pageNo: any) { 	
    ...
    this.http.get(`https://danshopapi.devworktdmc.com/commercial/get_members?pageNumber=${this.currentIndex}&recordsPerPage=${this.pageSize}&search=${this.inputName}`).subscribe((response) => {
