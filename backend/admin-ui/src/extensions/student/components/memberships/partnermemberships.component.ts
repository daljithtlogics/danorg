...
@Component({
  selector: 'partner',
  templateUrl: './partnermemberships.component.html',					
  styleUrls: ['./membershipcommon.component.css'],      
  standalone: true,
  imports: [SharedModule],
})

export class PartnermembershipsComponent implements OnInit 
{
  greeting = 'partner';  
  pageSize : number;  
  maxPages: number;		 
  ...
  getData(pageNo: any) { 	
    ...
    this.http.get(`https://danshopapi.devworktdmc.com/partner/get_members?pageNumber=${this.currentIndex}&recordsPerPage=${this.pageSize}&search=${this.inputName}`).subscribe((response) => {
