...
@Component({
    selector: 'editprofessional',			
    templateUrl: './editprofessionalmembership.component.html',		 		 				  								
	styleUrls: ['./editprofessionalmembership.component.css'],     		      	   	            
    standalone: true,
    imports: [SharedModule],		
})
export class EditProfessionalMembershipComponent implements OnInit {     
	  greeting = 'edit professional';	 	 		  				  
	  id:any; 
	  broker_id:any; 
	  profForm:any; 	     
	  form!: FormGroup;    
      ...
      comments: any;     
	  data: any; 	  
	  private apiUrl = 'https://danshopapi.devworktdmc.com/professional/update'; // Replace this with your API endpoint
	  
	  constructor(private formbulider: FormBuilder,private http: HttpClient,private route: ActivatedRoute) {  		
		this.id = this.route.snapshot.paramMap.get('id');     												
		this.member_type='PROFESSIONAL';  
	  ...
	  ngOnInit() {
			...
			this.profForm = this.formbulider.group({ 				 		
	  ...
	  onSelect(event: any) {
		...
		{ 			
			this.prof_type=this.prfn_type;   
			this.professional='Yes'; 	
			this.profForm.controls['prof_type'].setValidators([Validators.required]); 
		}
		else
		{
			this.prof_type="";	  
			this.professional='No'; 	  
			this.profForm.controls['prof_type'].clearValidators();  		
		}
		
		this.profForm.controls['prof_type'].updateValueAndValidity();			
 ...
 // evt.currentTarget.className += " active";
  ...
  updateProfessional(id:any,data:any): Observable<any> {
  ...
  onSubmit() { 
	...
	{ 		  		   	
		...
		const form_data = this.profForm.value;   				
		
		this.updateProfessional(this.id,form_data).subscribe(	  	  	
			response => {
			  console.log('Success:', response);     
			  this.successMsg='Record Updated';    
			  this.profForm.controls['main_reason'].reset();   	  	   		  		 	
			  this.profForm.controls['sub_reason'].reset();      		  
			  this.profForm.controls['comment'].reset();   	
			},
			error => {
  ...
  getData(id) { 
	  this.http.get('https://danshopapi.devworktdmc.com/professional/edit/'+id).subscribe((response) => {
      this.data = response;   	  
	  ...
	  this.prfn_type=(this.data.length >0)?this.data[0].professional_type:'';  	
	  this.prof_type=this.prfn_type;  
  ...
  getComment(id)    
  {
	  this.http.get('https://danshopapi.devworktdmc.com/professional/comments/'+id).subscribe((response) => {
