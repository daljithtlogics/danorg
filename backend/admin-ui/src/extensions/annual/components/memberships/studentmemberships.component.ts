...
@Component({
  selector: 'student',
  templateUrl: './studentmemberships.component.html',
  styleUrls: ['./membershipcommon.component.css'],
  standalone: true,
  imports: [SharedModule],
})
export class StudentmembershipsComponent implements OnInit {
  greeting = 'Student';
  pageSize : number;  
  maxPages: number;		 
  id : number;     
  ...
  getData(pageNo: any) { 	
    ...
    this.http.get(`https://danshopapi.devworktdmc.com/student/get_members?pageNumber=${this.currentIndex}&recordsPerPage=${this.pageSize}&search=${this.inputName}`).subscribe((response) => {
