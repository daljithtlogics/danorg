...
@Component({
    selector: 'editnondive',    		
    templateUrl: './editnondivefamily.component.html',		  		  						
	styleUrls: ['./addannualmemberships.component.css'],       						  
    standalone: true,
    imports: [SharedModule],
})
export class EditNonDiveFamilyComponent implements OnInit {	          
      greeting = 'non dive family';       
	  id: any;  
	  tab_id:any;  
	  main_id:any; 	 
	  tab_label:any; 	  	
	  ...
	  paid:any;    
	  data: any;  	  
	  private apiUrl = 'https://danshopapi.devworktdmc.com/nondiver/update';  		  
	...
	constructor(private formbulider: FormBuilder,private http: HttpClient,private pageMetadataService: PageMetadataService,private route: ActivatedRoute) {
		...
		pageMetadataService.setBreadcrumbs([
            ...
            { link: ['./'], label: 'Edit Non Diving Family' },             			     		    		
        ]);       
    }		
  ...
  getData(id) { 
    this.http.get('https://danshopapi.devworktdmc.com/nondiver/edit/'+id).subscribe((response) => {   
      this.data = response;
	  ...
	  this.workdiver=(this.data.length >0)?this.data[0].work_diver:'';           
