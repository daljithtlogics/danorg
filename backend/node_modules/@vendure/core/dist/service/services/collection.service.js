...
let CollectionService = class CollectionService {
    ...
    async delete(ctx, id) {
        ...
        for (const coll of [...descendants.reverse(), collection]) {
            ...
            this.eventBus.publish(new collection_modification_event_1.CollectionModificationEvent(ctx, deletedColl, affectedVariantIds));
        }
        this.eventBus.publish(new collection_event_1.CollectionEvent(ctx, deletedCollection, 'deleted', id));
        return {
     ...
     */
    async getFilteredProductVariantIds(filters) {
        if (filters.length === 0) {
            return [];
        }
        const { collectionFilters } = this.configService.catalogOptions;
