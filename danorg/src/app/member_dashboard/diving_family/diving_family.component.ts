...
@Component({
  selector: 'diving-family-dashboard',
  templateUrl: './diving_family.component.html',
  styleUrls: ['./diving_family.component.css'],
})
  
export class DivingFamily implements OnInit {
  ...
  private apiUrl = 'https://danshopapi.devworktdmc.com/common_membership/add_diving_family';  		  
  ...
  ngOnInit(): void {
    ...
    if ((this.router.url == '/diving-family') && (this.userAccess !='paid') || (this.userRenew ==='renew'))      
    {
  ...
  updateDive(id:any,data:any): Observable<any> {
    return this.http.post<any>('https://danshopapi.devworktdmc.com/login_frontend/update_diving_family/'+id,data);	           		    							
  }  
  ...
  getDive(id:any)
  {
	  this.http.get('https://danshopapi.devworktdmc.com/common_membership/fetch_dives/'+id).subscribe((response) => {
		    this.dives = response;  
        this.getDive(this.userId);
	  });   
  }
  ...
  editDive(id:any)
  {
	  this.http.get('https://danshopapi.devworktdmc.com/common_membership/edit/'+id).subscribe((response) => {
        ...
        this.qualification=(this.edit_dives.length >0)?this.edit_dives[0].qualification:'';     	
        this.agency=(this.edit_dives.length >0)?this.edit_dives[0].agency:'';      
        this.workdiver=(this.edit_dives.length >0)?this.edit_dives[0].work_diver:'';  
        this.freediver=(this.edit_dives.length >0)?this.edit_dives[0].free_diver:''; 
	  });   
  }
